...
{
	$query = "UPDATE hase_game_list SET status='$status' WHERE game_id = '$game_id'";
	...
	if ($status == 1) //game started, setting positions ingame
	{
		...
		while ($row = mysql_fetch_array( $result ))
		{
			...
			$query = "UPDATE hase_player_list SET position_in_game='$position' WHERE game_id = '$game_id' AND player_id='$player_id'";
			mysql_query($query) or die;
			$i++;
		}
	}
